"""
Fuel index, 2026-09-10 (evening) — the EU Weekly Oil Bulletin's diesel price for a
country (first built for Estonia; since G2 any EU-27 tenant country, and a typed price
for any other), fetched server-side, cached in ONE global row per country, never invented.
...
# The countries the EU Weekly Oil Bulletin publishes a diesel row for (EU-27). A tenant
# outside this set — the UK since Brexit, Norway, Switzerland — has NO automatic index:
# ensure_fresh() and refresh() decline to fetch, the state says so, and the planner types
# the price (set_manual). G2, 16 Sep 2026.
BULLETIN_COUNTRIES = {
...
def auto_available(country):
    """Does the bulletin carry this country at all?"""
    return str(country or "").strip().upper() in BULLETIN_COUNTRIES
...
def refresh(country=DEFAULT_COUNTRY, sync=False, timeout=TIMEOUT_S):
    ...
    if not auto_available(country):
        # no feed to fetch: say so in the row's error slot, never mark a fetch as tried
        with _REFRESH_LOCK:
            _REFRESH.update({"running": False, "started_at": _now(), "finished_at": _now(),
                             "status": "manual_only",
                             "error": f"no automatic diesel index for {str(country).upper()} — type the price"})
...
def state(country=DEFAULT_COUNTRY, today=None):
    ...
    return {
        ...
        "auto_available": auto_available(country),
