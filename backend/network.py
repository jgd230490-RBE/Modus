...
# G2, 16 Sep 2026: the product ships NO network. seed_network(path) loads one from an
# explicit file (a test fixture, a migration tool); with no path it is a no-op, so a
# fresh tenant boots empty. Tenants are populated by tenant_package.import_tenant().
_SEED = None
...
def seed_network(path=None):
    """Load a network file (locations + routes) into an EMPTY tenant. No path: no-op."""
    if path is None:
        return False
...
_NODE_META = None   # G2: no shipped metadata; apply_node_meta(path) takes an explicit file


def backfill_forecast_ipt():
    """
    Task F, one-off and idempotent. A forecast line written before `ipt` existed gets
    its route's team — but ONLY where the route names exactly one ("IPT5"). A shared
    route reads "IPT3 / IPT6", and nothing on the line says which of the two it is,
...
def apply_node_meta(path=None):
    """
    Write vendor / material detail from a metadata file onto locations that lack it.
    Additive only: a row whose vendor is already set is left alone. No path: no-op.
    (Historically a boot-time salvage of one project's quarry operators; since G2 the
    product ships no such file and this is a tool for an explicit one.)
    """
    if path is None:
        return {"applied": 0, "skipped": 0, "note": "no metadata file given"}
    try:
        with open(path, encoding="utf-8") as f:
            payload = json.load(f)
    except Exception:
        return {"applied": 0, "skipped": 0, "note": "metadata file not present"}
    ...
    for l in db.query("SELECT * FROM locations WHERE tenant_id = ? ORDER BY id",
        ...
        feats.append({
            ...
            "properties": {
                ...
                # two sites with one name, so the id rides along in the label
                "name": l["name"],
...
def meta_routes(profile=None):
    ...
    Three things the raw `routes` table cannot give the UI, all handled here:
      ...
      duplicates  two locations may share a name (the first network had a pair). The
