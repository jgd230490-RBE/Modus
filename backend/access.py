...
def configured_codes():
    ...
    for i in range(1, 7):
        ...
        if c:
            # the label is what a person reads — the tenant's name for the slot (G2)
            out[c] = {"role": "ipt", "ipt": f"IPT{i}", "label": _team_label(f"IPT{i}")}
...
def _team_label(team_id):
    try:
        import taxonomy
        return taxonomy.team_name(team_id)
    except Exception:
        return team_id


def tenant_team_ids():
    """The slots this tenant names, else all six (a tenant with no team rows yet)."""
    try:
        import taxonomy
        ids = [canonical_ipt(i) for i in taxonomy.team_ids()]
        ids = [i for i in ids if i]
        return ids or list(IPT_IDS)
    except Exception:
        return list(IPT_IDS)
