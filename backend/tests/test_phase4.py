...
# G2: the zone is the TENANT's, from its country — no longer one project's fixed zone
_t_gb, _n_gb = here_routing.default_departure_times(now=_now, country="GB")
_t_ee, _n_ee = here_routing.default_departure_times(now=_now, country="EE")
_t_xx, _n_xx = here_routing.default_departure_times(now=_now, country="")
ok("G2: a GB tenant probes on London time, an EE tenant on Tallinn time",
   ("Europe/London" in _n_gb or "ASSUMED" in _n_gb) and ("Europe/Tallinn" in _n_ee or "ASSUMED" in _n_ee)
   and (_t_gb[0][-6:] in ("+01:00", "+00:00") or "ASSUMED" in _n_gb), (_n_gb, _t_gb[0], _n_ee))
ok("G2: no country → UTC, and the note says it was assumed",
   "UTC ASSUMED" in _n_xx and _t_xx[0].endswith("+00:00"), (_n_xx, _t_xx[0]))
