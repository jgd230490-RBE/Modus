...
# G2: this harness models a tenant in the provider's country (the restriction layer
# is Estonia's road authority; a tenant elsewhere has no provider and these paths
# answer "none" — asserted in test_modus.py).
_rx.COUNTRY_OVERRIDE = "EE"
...
ok("⭐ ...sheet 1 is WEEKDAY × line: one row per Mon–Fri day per visible line, headers from the export mock",
   # G2 (16 Sep): the fifth header is the tenant's team_label — "Team" on a default tenant
   ws.max_row - 1 == 3 * NWD and [c.value for c in ws[1]][:6] == ["Date", "Route", "Origin", "Destination", "Team", "WS"])
