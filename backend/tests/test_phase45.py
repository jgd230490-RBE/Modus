...
with as_tenant(A):
    seeded_a = network.seed_network(FIX_NET)
    n_loc_a = db.count_locations()
with as_tenant(B):
    seeded_b = network.seed_network(FIX_NET)
...
with as_tenant(A):
    ok("re-seeding an already-seeded tenant is still a no-op",
       network.seed_network(FIX_NET) is False)
...
ok("⭐ and BEFORE the taxonomy seed, which must write into the tenanted table",
   main_src.index("db.init_tenant()") < main_src.index("taxonomy.seed_taxonomy()"))
ok("G2: the startup seeds no network at all — a fresh tenant boots empty",
   "network.seed_network(" not in "\n".join(l.split("#", 1)[0] for l in main_src.splitlines()))
