...
counts = taxonomy.seed_taxonomy(taxonomy.load_bundle(FIX_TAX))
...
again = taxonomy.seed_taxonomy(taxonomy.load_bundle(FIX_TAX))
...
# =========================================================================== #
res = network.apply_node_meta(FIX_META)
...
again = network.apply_node_meta(FIX_META)
...
# G2: the seed module itself is gone, and so is the boot-time network seed. The product
# boots to an EMPTY tenant; the package importer is the only way rows arrive.
ok("G2: the product ships no seed module", not os.path.exists(os.path.join(BACKEND, "seed.py")))
ok("G2: the product ships no seed_data directory",
   not os.path.exists(os.path.join(BACKEND, "seed_data")))
ok("G2: the startup does not seed a network", "network.seed_network(" not in main_code)
ok("G2: the startup applies no node metadata", "apply_node_meta(" not in main_code)
ok("G2: seed_network() with no path is a no-op", network.seed_network() is False)
ok("G2: apply_node_meta() with no path is a no-op", network.apply_node_meta()["applied"] == 0)
