...
# Tests that model a tenant in the provider's country without a config table set this;
# the product never does. (test_phase25a.py, test_lookahead.py)
...
def tenant_country():
    ...
    try:
        ...
        return _config.tenant_settings(_conversions).get("country")
    except Exception:
        return None
...
def _none_fc(note):
    return {"type": "FeatureCollection", "features": [], "layers": [], "errors": {},
            "counts": {}, "current_only": True, "total_records": 0, "expired_or_future": 0,
            "attribution": None, "provider": None, "note": note}


NO_PROVIDER_NOTE = "no road-restriction provider for this tenant's country"
...
def check_all(profile=None, layers=None):
    ...
    """
    if not enabled():
        return {"routes": {}, "route_count": 0, "provider": None, "note": NO_PROVIDER_NOTE}
    fc = fetch_all(layers)
...
def store_checks(route_ids=None, profile=None):
    ...
    if not enabled():
        # a tenant with no provider is CLEAN by absence, not unavailable: the stored
        # status says why, and the clash rail shows no restriction source at all
        now = _iso_now()
        for rid in ids:
            db.execute("UPDATE routes SET restrictions_hits = ?, restrictions_checked_at = ?, "
                       "restrictions_status = ? WHERE tenant_id = ? AND id = ?",
                       (json.dumps([]), now, "no_provider", tenant, rid))
        return {"status": "no_provider", "routes": len(ids), "hits": 0, "checked_at": now,
