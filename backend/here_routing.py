...
# The tenant's local time zone, for the departure-time probe. A tenant's country is set
# on Config (tenant.country); a country missing here falls back to UTC and says so.
COUNTRY_TZ = {
    "GB": "Europe/London", "IE": "Europe/Dublin", "EE": "Europe/Tallinn", "LV": "Europe/Riga",
    "LT": "Europe/Vilnius", "FI": "Europe/Helsinki", "SE": "Europe/Stockholm", "NO": "Europe/Oslo",
    "DK": "Europe/Copenhagen", "PL": "Europe/Warsaw", "DE": "Europe/Berlin", "NL": "Europe/Amsterdam",
    "BE": "Europe/Brussels", "FR": "Europe/Paris", "ES": "Europe/Madrid", "PT": "Europe/Lisbon",
    "IT": "Europe/Rome", "AT": "Europe/Vienna", "CH": "Europe/Zurich", "CZ": "Europe/Prague",
}
...
def _tenant_country():
    try:
        import config
        import conversions
        return config.tenant_settings(conversions).get("country")
    except Exception:
        return None


def default_departure_times(now=None, country=None):
    """
    ...
    country = (country if country is not None else _tenant_country()) or ""
    zone = COUNTRY_TZ.get(str(country).upper())
    try:
        if not zone:
            raise LookupError("no zone for this country")
        from zoneinfo import ZoneInfo
        tz, note = ZoneInfo(zone), f"{zone} from the system tz database (tenant country {country})"
    except Exception:
        # ⚠️ Labelled as an assumption: without the tenant's zone the probe runs on UTC.
        tz, note = timezone.utc, (f"UTC -- no time zone known for country {country or 'unset'}, UTC ASSUMED")
