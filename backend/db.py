...
def init_network_db():
    ...
    try:
        ...
        #     Work sections OVERLAP (a station can be a point section inside a band inside a corridor-wide row),
...
def init_taxonomy_db():
    ...
    Two flags on both disciplines and work_sections, and they are NOT the same question:
      active   — does this exist in the current org configuration? (a team merged into another: no)
      in_scope — does this tenant haul to it? (a design-only section, or a discipline another
                 contractor supplies: no)
    ...
    is a MAINLINE rule, not a design-section rule — a design section can also contain
    a terminal, a depot and a maintenance facility where the scope is fuller. Scope
...
_TENANT_DDL = {
    "forecasts": """
        CREATE TABLE forecasts (
            ...
            -- can read "IPT3 / IPT6" on a shared route, and not the section. NULL on
